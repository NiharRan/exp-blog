...
// process.env.IMAGE_STORAGE contains uploads/posts
var UPLOAD_PATH = path.resolve(__dirname, "..", process.env.IMAGE_STORAGE);

// create a multer storage engine
var Storage = function (options) {
  // this serves as a constructor
  function Storage(opts) {
    var baseUrl = process.env.IMAGE_BASE_URL;
    ...
    if (this.options.storage == "local") {
      ...
      !fs.existsSync(this.uploadPath + "/lg") &&
        mkdirp.sync(this.uploadPath + "/lg");
      !fs.existsSync(this.uploadPath + "/md") &&
        mkdirp.sync(this.uploadPath + "/md");
      !fs.existsSync(this.uploadPath + "/sm") &&
        mkdirp.sync(this.uploadPath + "/sm");
    }
  }

  // this generates a random cryptographic filename
  Storage.prototype._generateRandomFilename = function () {
    // create pseudo random bytes
    var bytes = crypto.pseudoRandomBytes(10);
  ...
  Storage.prototype._createOutputStream = function (filepath, cb) {
  ...
  Storage.prototype._processImage = function (image, cb) {
    // create a reference for this to use in local functions
    var that = this;
    ...
    if (this.options.responsive) {
      ...
      batch = _.map(sizes, function (size) {
        ...
        var image = null;
        var filepath = path.join(that.uploadPath + "/" + size, filename);
  ...
  Storage.prototype._removeFile = function (req, file, cb) {
